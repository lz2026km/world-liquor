...
async function main() {
  ...
  // Block external fonts (they timeout)
  await page.route('**/fonts.googleapis.com/**', route => route.abort());
  await page.route('**/fonts.gstatic.com/**', route => route.abort());
  
  // Intercept baijiu_data.json using regex to catch all protocols (file://, http://, etc.)
  await page.route(/\/baijiu_data\.json/, async route => {
    const body = fs.readFileSync(path.join(__dirname, 'baijiu_data.json'));
    console.log('[INTERCEPT] baijiu_data.json served');
  ...
  console.log('卡片tabindex: ' + tab + (tab !== null ? ' ✅' : ' ⚠️'));
  p++;
